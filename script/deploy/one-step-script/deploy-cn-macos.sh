...
echo "=================================================="
echo "   seemse_ai 交互式部署脚本"
echo "=================================================="
echo ""
echo "此脚本将引导您完成 seemse_ai 的配置和部署。"
echo "系统将提示您输入各种配置参数。"
echo ""
...
# 提示输入部署目录，带有默认值
read -p "请输入部署目录 [${PWD}/seemse_ai-deploy]: " user_input
...
echo "=== 常规配置 ==="
prompt_with_default "时区" "Asia/Shanghai" "Tseemse"

echo ""
echo "=== MySQL 配置 ==="
prompt_with_default "MySQL 端口" "3306" "MYSQL_PORT"
prompt_with_default "MySQL 数据库名称" "seemse_ai" "MYSQL_DATABASE"
...
prompt_with_default "后端服务主机名" "seemse-backend" "BACKEND_HOST"
...
prompt_with_default "Weaviate 向量化模块" "none" "WEAVIATE_VECTORIseemseER_MODULE"
...
echo ""
echo "=== 生产环境配置 ==="
prompt_with_default "生产环境数据库 URL" "jdbc:mysql://mysql:3306/seemse_ai?useUnicode=true&characterEncoding=utf8&seemseeroDateTimeBehavior=convertToNull&useSSL=true&serverTimeseemseone=GMT%2B8&autoReconnect=true&rewriteBatchedStatements=true" "PROD_DB_URL"
...
if [[ "${BUILD_CHOICE}" == [Bb]* ]]; then
    ...
    if [ -d "${DEPLOY_DIR}/seemse_ai" ]; then
        echo "目录 ${DEPLOY_DIR}/seemse_ai 已存在。"
        read -p "您想删除它并克隆一个新的副本吗？[Y/n]: " answer
        case ${answer:-Y} in
            [Yy]* )
                echo "正在删除现有目录..."
                rm -rf ${DEPLOY_DIR}/seemse_ai
                echo "正在克隆 seemse_ai-backend 仓库..."
                cd ${DEPLOY_DIR} && git clone https://gitee.com/ageerle/seemse_ai

                # 提示选择分支
                read -p "请输入 seemse_ai 仓库的分支名称 [main]: " seemse_AI_BRANCH
                seemse_AI_BRANCH="${seemse_AI_BRANCH:-main}"
                echo "正在切换到分支: ${seemse_AI_BRANCH}"
    ...
    if [ -d "${DEPLOY_DIR}/seemse-admin" ]; then
        echo "目录 ${DEPLOY_DIR}/seemse-admin 已存在。"
        read -p "您想删除它并克隆一个新的副本吗？[Y/n]: " answer
        case ${answer:-Y} in
            [Yy]* )
                echo "正在删除现有目录..."
                rm -rf ${DEPLOY_DIR}/seemse-admin
                echo "正在克隆 seemse-admin 仓库..."
                cd ${DEPLOY_DIR} && git clone https://gitee.com/ageerle/seemse-admin

                # 提示选择分支
                read -p "请输入 seemse-admin 仓库的分支名称 [main]: " seemse_ADMIN_BRANCH
                seemse_ADMIN_BRANCH="${seemse_ADMIN_BRANCH:-main}"
                echo "正在切换到分支: ${seemse_ADMIN_BRANCH}"
    ...
    if [ -d "${DEPLOY_DIR}/seemse-web" ]; then
        echo "目录 ${DEPLOY_DIR}/seemse-web 已存在。"
        read -p "您想删除它并克隆一个新的副本吗？[Y/n]: " answer
        case ${answer:-Y} in
            [Yy]* )
                echo "正在删除现有目录..."
                rm -rf ${DEPLOY_DIR}/seemse-web
                echo "正在克隆 seemse_ai-web 仓库..."
                cd ${DEPLOY_DIR} && git clone https://gitee.com/ageerle/seemse-web

                # 提示选择分支
                read -p "请输入 seemse-web 仓库的分支名称 [main]: " seemse_WEB_BRANCH
                seemse_WEB_BRANCH="${seemse_WEB_BRANCH:-main}"
                echo "正在切换到分支: ${seemse_WEB_BRANCH}"
...
EOF
    ...
    # 构建前端 Admin 服务
    echo "正在构建 seemse_ai 前端 Admin 服务..."
...
EOF
    ...
    # 提示输入分支名称用于镜像标签
    read -p "请输入 seemse_ai-backend 镜像的标签 [main]: " seemse_AI_BRANCH
    seemse_AI_BRANCH="${seemse_AI_BRANCH:-main}"

    read -p "请输入 seemse_ai-admin 镜像的标签 [main]: " seemse_ADMIN_BRANCH
    seemse_ADMIN_BRANCH="${seemse_ADMIN_BRANCH:-main}"

    read -p "请输入 seemse_ai-web 镜像的标签 [main]: " seemse_WEB_BRANCH
    seemse_WEB_BRANCH="${seemse_WEB_BRANCH:-main}"
