...
└── deploy/
    └── deploy/
        └── mysql-init/
            └── seemse_ai.sql  # Docker初始化SQL（与主文件同步）
```
...
**【强制】** 首次初始化项目只需要执行：
```bash
mysql -u root -p database_name < script/sql/seemse_ai.sql
```
...
2. 将变更同步更新到初始化文件 `script/sql/seemse_ai.sql`
3. 同步更新Docker初始化文件 `script/deploy/deploy/mysql-init/seemse_ai.sql`
